...
Функция ПодготовитьЭлектронныйДокумент(Результат, ВидДокументаРазвернутый, ДополнительныеПараметры)
	...
	ИначеЕсли ВидДокументаРазвернутый.ID = "ID_УПД_ДОП" Тогда
		
		Допсведения = Неопределено;
		ФИОПодписанта = " ";
		Заполнить_UtdSellerContent(Результат.Content, Новый Структура("ДокументСсылка, Function", Документ1С, "Basic"));
...
Функция ПолучитьТекстЗапросаДляВидаПакета(ВидПакетаРазвернутый)
	...
	Если ВидПакетаРазвернутый.ID = "ID_ОсновнойПакет" Тогда
		...
		|		соединение Документ.РеализацияТоваров как РеализацияТоваров
		|			по СчетФактураВыданный.ДокументОснование = РеализацияТоваров.Ссылка
		|		соединение документ.ОтгрузочныйЛист как ОтгрузочныйЛист
		|			по РеализацияТоваров.Ссылка = ОтгрузочныйЛист.ДокументОснование
		|			и ОтгрузочныйЛист.СтатусОтгрузки = ЗНАЧЕНИЕ(Перечисление.СтатусыОтгрузки.Отгружен)
		|ГДЕ
...
Функция ПодготовитьПакет(ВидПакетаРазвернутый, СтрокаСписка, Пакет)
	...
	Если ВидПакетаРазвернутый.ID = "ID_ОсновнойПакет" Тогда
		...
		ОсновнойМодуль.ЭДО_ДокументМенеджер_ПодготовитьИДобавитьДокументВПакет(Пакет,	СтрокаСписка.Документ.ДокументОснование, "ID_УПД_ДОП");
...
Процедура Заполнить_UtdSellerContent(Контент,Параметры)
	...
	Иначе
		ДанныеДляПечати = Документы.РеализацияТоваров.ПолучитьДанныеДляПечатнойФормыУПД(Новый Структура, НовыйМассив(ДокументСсылка));
	КонецЕсли;
	...
	ЗаполнитьРеквизитыПодвалаУПД(Контент, ДанныеПечати, СведенияОбОрганизации, СведенияОКонтрагенте, ДокументСсылка);

КонецПроцедуры

Функция ПолучитьНомерДатуДоговора(договор)
	...
	//
	//Если Сред(наименование, длина - 4, 1) = "." и Сред(наименование, длина - 7, 1) = "." Тогда
	//	результат
	
КонецФункции

Функция СформироватьСтруктуруФИО(СтрокаФИО)
	
	результат = Новый Структура("Фамилия,Имя,Отчество");
	стр = СокрЛП(СтрокаФИО);
	поз = СтрНайти(стр, " ");
	Если поз = 0 тогда
		ВызватьИсключение "ФИО (через пробел) ответственного за передачу должно быть заполнено";
	КонецЕсли;
	
	результат.Фамилия = лев(стр, поз);
	...
	Конецесли;
	Если поз <> 0 тогда
		результат.Отчество = лев(стр, поз);
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыПодвалаУПД(Контент, ДанныеПечати, СведенияОбОрганизации, СведенияОКонтрагенте, ДокументСсылка)
	
	РеализацияТоваровСсылка = ?(ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваров"), ДокументСсылка, ДокументСсылка.ДокументОснование);	
	
	УстановитьЗначениеXDTO(Контент.TransferInfo, "OperationInfo", "Товары переданы, работы сданы, услуги оказаны");
	
	ТипСтроки = Контент.TransferInfo.Свойства().Получить("TransferBases").Тип.Имя;
	НовЭлемент = Контент.TransferInfo.TransferBases.Добавить(НовыйКонтент(ТипСтроки));
	УстановитьЗначениеXDTO(НовЭлемент, "DocumentName",						"договор");
	УстановитьЗначениеXDTO(НовЭлемент, "DocumentNumber", РеализацияТоваровСсылка.ДоговорВзаиморасчетов.Договор2006.Наименование);
	УстановитьЗначениеXDTO(НовЭлемент, "DocumentDate", РеализацияТоваровСсылка.ДоговорВзаиморасчетов.ДатаДоговора);
	
	УстановитьЗначениеXDTO(Контент.TransferInfo, "OperationInfo", "Товары переданы, работы сданы, услуги оказаны");
	УстановитьЗначениеXDTO(Контент.TransferInfo, "TransferTextInfo", "" + ДанныеПечати.ОЛВсегоМест + " мест");
	
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваров") Тогда
		УстановитьЗначениеXDTO(Контент.TransferInfo, "TransferDate", ДанныеПечати.Дата);
		
		Рез = Документы.СчетФактураВыданный.СформироватьДанныеПоДоверенностямОрганизаций(ДанныеПечати);
		Если Не Рез.Пустой() Тогда
			Оператор = Рез.Выгрузить();
			ОтвЛицо = Оператор[0].ФамилияИО  + " " + Оператор[0].Приказ;
			структураФИО = СформироватьСтруктуруФИО(Оператор[0].ФамилияИО);
			УстановитьЗначениеXDTO(Контент.TransferInfo.Employee, "EmployeePosition", Оператор[0].Должность);
			УстановитьЗначениеXDTO(Контент.TransferInfo.Employee, "EmployeeBase", Оператор[0].Приказ);
			УстановитьЗначениеXDTO(Контент.TransferInfo.Employee, "Surname", структураФИО.Фамилия);
			УстановитьЗначениеXDTO(Контент.TransferInfo.Employee, "Name", структураФИО.Имя);
			УстановитьЗначениеXDTO(Контент.TransferInfo.Employee, "Patronymic", структураФИО.Отчество);
		КонецЕсли;
	КонецЕсли;
